...
def update_voice_sample(voice):
    sample_path = os.path.join("/home/nico/OpenClaw_V2/plugins/anime_video_generator/assets/samples", f"{voice}.wav")
...
def generate_video(youtube_url, streamer_name, custom_hook, bgm_choice, tts_voice, enable_montage):
    ...
    # Try to return the Rank1 video to the UI specifically
    for f in mp4_files:
        if "Rank1" in f:
            final_video = f
            break
    ...
    logs += "\n✅ Success! All 5 Ranked Videos and Thumbnails generated successfully and saved to outputs/ directory."
...
with gr.Blocks(title="Viral Shorts Generator Pro") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            voice_sample = gr.Audio(value="/home/nico/OpenClaw_V2/plugins/anime_video_generator/assets/samples/am_onyx.wav", label="Voice Preview", interactive=False)
            montage_check = gr.Checkbox(label="Enable 8-Second Phonk Beat-Sync Montage (2s Front Loop, 6s Back)", value=True)
