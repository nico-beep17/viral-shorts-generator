...
BGM_FILE=$(find assets/bgm -type f -name "*.mp3" | shuf -n 1 2>/dev/null)
if [ -z "$BGM_FILE" ]; then
...
ffmpeg -y -i clip_fixed.mp4 -i clip_panned.mp4 -i narration.wav -i "$BGM_FILE" -filter_complex "
    ...
    drawtext=text='${HOOK_TITLE}':fontcolor=white:fontsize=64:borderw=6:bordercolor=black:shadowcolor=black:shadowx=4:shadowy=4:box=1:boxcolor=red@0.8:boxborderw=15:x=(w-text_w)/2:y=300:fontfile=/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf,
    drawtext=text='${STREAMER_NAME}':fontcolor=yellow:fontsize=56:borderw=5:bordercolor=black:shadowcolor=black:shadowx=3:shadowy=3:box=1:boxcolor=black@0.8:boxborderw=10:x=(w-text_w)/2:y=400:fontfile=/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf[vsub];
